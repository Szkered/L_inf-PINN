...
def train(rank, world_size, config):
  ...
  layers = [cfg.equation.x_dim + 1
           ] + [cfg.model.width] * (cfg.model.depth - 1) + [1]
  ...
  for i in range(cfg.train.iteration):
    ...
    if rank == 0:
      logger.info(
        f'iteration {i}| loss {loss.detach().cpu().item():.5f}\t| '
      ...
      wandb.log(
        dict(
          ...
          domain_loss=dloss.detach().cpu().item(),
          boundary_loss=bloss.detach().cpu().item()
        ),
        step=i
      )
    ...
    if rank == 0 and (i + 1) % cfg.test.step == 0:
      ...
      i_avg_err, i_rel_err_l1 = test(
        cfg, f, test_X, test_Y, rank, norm_type='l1'
      )
      logger.info(
        f'L1 test error: average {i_avg_err}, relative {i_rel_err_l1}'
      )
      i_avg_err, i_rel_err_l2 = test(
        cfg, f, test_X, test_Y, rank, norm_type='l2'
      )
      logger.info(
        f'L2 test error: average {i_avg_err}, relative {i_rel_err_l2}'
      )
      wandb.log(dict(l1_rel=i_rel_err_l1, l2_rel=i_rel_err_l2), step=i)
      i_avg_err, i_rel_err = test(
        cfg,
        f,
        test_X,
        torch.concat([test_Y[:, None], test_grad_x], dim=1),
        rank,
        norm_type='w11'
      )
